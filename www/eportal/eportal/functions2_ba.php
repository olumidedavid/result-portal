...
function semesterOptions($db, $currsess=0, $current='')
{
    ...
    $res = $db->query("SELECT s.sessionName
                     ...
                     --   $where
                        ORDER BY s.date_from, t.date_from     
                       ");
    $prevsess = '';
    foreach ($res as $r) {
        if ($current) {
        ...
        $opts .= "<option $sel value='{$r['id']}'>{$r['semesterName']}</option>\n";
    }
    return $opts;
}
...
function studentOptions($pdo, $semester, $class, $staff, $current='')
{
    ...
    $res->execute([ $semester, $class, $staff ]);
...
function classAverageScores($pdo, $class, $session, $term)
{
    $res = $pdo->prepare("SELECT   c.subjectid
                                 , round(sum(score) / count(distinct studentid) / count(distinct semesterid), 0) as ave
                            FROM result r
