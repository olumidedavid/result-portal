    ...
    private function getData($student)
    {
        ...
        if ($r) {
            ...
            foreach ($this->data as $subid => &$subdata) {
                $total = round(($subdata['last'] + $subdata['ca'])/$this->termno);
                $subdata['last'] = $total;
                $grand_total += $total;
                if ($total > 0) {
...
{
    protected $report_title = 'End of Term Results';
    ...
    private function getData($student)
    {
        ...
        $res = $this->pdo->prepare("SELECT COUNT(DISTINCT stc.studentid) AS pupils
                            ...
                            WHERE sm.sessionid = ?
                              AND stc.classid = ? 
                            ");
        $res->execute([ $this->sessionid, $this->studentclassid ]);
        $this->pupilsinclass = $res->fetchColumn();    
