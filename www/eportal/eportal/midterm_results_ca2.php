...
//                                                                                
//$_SESSION['staff_id'] = 16;                                                         # testing only
//$_SESSION['student_id'] = 681;                                                       # testing only
...
################################################################################
    ...
    $res = $pdo->prepare("SELECT st.id as stid
                                 , concat_ws(' ', st.firstname, st.othername, st.lastname) as stname
                                 , st.image
                                 , cl.classname
                                 , sc.classid
                                 , l.id as level
                            ...
                            FROM result r
                                 ...
                                 ) ON r.studentclassid = sc.id AND exam IN ('CA1', 'CA2') and r.courseid = c.id                            ### 10/06/23
                            WHERE sn.id = ?
                              AND studentid = ?
        ...
        do {
            if (!isset($data[ $r['subjectid'] ])) {
                $data[ $r['subjectid'] ] = [ 'name' => $r['subjectname'],
                                             'ca' => ['CA1' => 0, 'CA2' => 0],                                                             ### 10/06/23
                                             'last'  => 0,
                                             'avg' => 0, 
                                             'terms' => 0
                                           ];
            }   
            $data[ $r['subjectid'] ]['ca'][$r['exam']] = $r['ca'];                                                                         ### 10/06/23
            //$subject_count += ($r['ca'] > 0);                                                                                            ### 10/06/23
        } while ($r = $res->fetch());
        $subject_count = count($data);                                                                                                     ### 10/06/23
...
//        if ($subject_count==0) $subject_count = 1;
        foreach ($data as $subid => $subdata) {
            $tdata .= "<tr><td>$n</td>
                           <td>{$subdata['name']}</td>";
            foreach ($subdata['ca'] as $exm) {                                                                                             ### 10/06/23
                $tdata .= "<td>$exm</td>";                                                                                                 ### 10/06/23
            }                                                                                                                              ### 10/06/23
            
        ##  $total = round(($subdata['last'] + $subdata['ca'])/($subdata['terms']+1));               // this line replaced by next
            $total = array_sum($subdata['ca']);                                                                 // replacement line        ### 10/06/23
...
-->
        ...
        <table border='0'  id='result-tbl'>
            <tr class='w3-border-bottom w3-dark-gray'>
                ...
                <th>Subject</th>
<th>CA1<br> (50)</th>
                <!--<th>&nbsp;&nbsp;&nbsp;&nbsp;CA1<br> &nbsp;&nbsp;(50)&nbsp;&nbsp;</th> -->                                                              <!--### 10/06/23-->
              <!--  <th>&nbsp;&nbsp;&nbsp;&nbsp;CA2<br> &nbsp;&nbsp;(50)&nbsp;&nbsp;</th> -->
<th>CA2<br> (50)</th>                                                              <!--### 10/06/23-->
                <th>Total <br>(100)</th>
                <th>Class<br>Avg</th>
                <th>Grade</th>
                <th>Comment</th>
