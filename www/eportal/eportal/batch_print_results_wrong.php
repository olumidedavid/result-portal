...
//        $this->Cell(120, 8, $this->report_title, 0, 2, 'C');
    ...
    protected function printResultsHeader()
    {
        ...
        $this->setFontSize(9);
        //$this->Cell($widths[2], 6, "Pupils in class", 0, 0, 'C', 1);
        $this->Cell($widths[2]);
        $this->Cell($widths[3]);
...
// Check if $t is a valid key in $th
if (isset($th[$t])) {
    $this->Cell($widths[0], 5, '', 'R', 0, 'C', 1);
    ...
    // Check if keys exist in the $th array before accessing them
    $cell6 = isset($th[$t][0][0]) ? $th[$t][0][0] : '';
    $cell7 = isset($th[$t][1][0]) ? $th[$t][1][0] : '';
    $cell8 = isset($th[$t][2][0]) ? $th[$t][2][0] : '';

    $this->Cell($widths[6], 5, $cell6, 'L', 0, 'C', 1);
    $this->Cell($widths[7], 5, $cell7, 0, 0, 'C', 1);
    $this->Cell($widths[8], 5, $cell8, 'R', 0, 'C', 1);
    ...
    private function printResultsTable($widths)
{
    ...
    foreach ($this->data as $subdata) {
        ...
        $s3 = $this->termno > 2 ? $subdata['scores'][3] : '';
        $this->Cell($widths[8], 6, $s3, 'R', 0, 'C', $n % 2);
...
if (!isset($_GET['result'])) {
    header("Location: create_pdfs.php");
    exit;
}
